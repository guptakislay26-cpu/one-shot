# LAIO SaaS Developer Notes

## Commands
- `npm run dev` starts backend on port 3000 and frontend on port 5173.
...
## Feature Pattern
Feature modules live under `backend/src/modules/<feature>/` with module, controller, service, and DTO files. Services talk directly to Prisma, enforce state machines in service methods, and use per-class Nest loggers.

Frontend server state goes through `src/lib/api.ts` and one hook module per feature. Global auth, UI, and offline queues use Zustand stores. The authenticated workspace is available at `/app`.
